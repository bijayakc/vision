...
class vxcf_ccontact_api extends vxcf_ccontact{
    ...
    public $auth_url='https://idfed.constantcontact.com/as/token.oauth2';
    public $api_key='';
    public $error= "";
    public $timeout= "15";
...
public function refresh_token($info=""){
  ...
  $re=$this->post_crm($this->auth_url,'post',$body,'url');
...
public function handle_code(){
  ...
  $token=$this->post_crm($this->auth_url,'post',$body,'url');
  }
  }
...
'email_permission'=>array('label'=>'Email Permissions','name'=>'email_permission',
...
'address_type'=>array('label'=>'Address - Type','name'=>'address_type','type'=>'Choice',
...
$status=''; $error=$arr[0]['error_message'];
}
...
public  function post_crm_arr($path,$method,$body=""){
  ...
  if(!empty($info['refresh_token']) && $sales_response['error_key'] == 'unauthorized'){ 
  $get_token=true;         
  }
...
if(is_array($body)&& count($body)>0 && ($type == '' && $method !='get')){ 
