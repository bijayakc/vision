<?php
/**
* Plugin Name: Contact Form 7 Constant Contact
* Description: Integrates Contact Form 7 and <a href="https://wordpress.org/plugins/contact-form-entries/">Contact Form Entries Plugin</a> with Constant Contact allowing form submissions to be automatically sent to your Constant Contact account 
* Version: 1.0.3
...
* Author: CRM Perks.
* Text Domain: contact-form-ccontact
...
class vxcf_ccontact {
 ...
 public  $url = "https://www.crmperks.com";
  ...
  public static $title='Contact Form Constant Contact Plugin';  
  public static $path = ''; 
  public static $slug = "";
...
load_plugin_textdomain('contact-form-ccontact-crm', FALSE,  $this->plugin_dir_name(). '/languages/' );
  ...
  $install=new vxcf_ccontact_install();
...
$this->push($entry,$form,'',false);    
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo $id ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Constant Contact Plugin','contact-form-ccontact-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php _e('Dismiss Notice','contact-form-ccontact-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo $id ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Constant Contact Plugin','contact-form-ccontact-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sContact Form Entries%s is required. %s it today!%s", 'contact-form-ccontact-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form Constant Contact Plugin','contact-form-ccontact-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php'), 'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'contact-form-ccontact-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");
  }  
...
return $objects;
  ...
  */
  ...
  $link="N/A"; $desc=__("Added to ",'contact-form-ccontact-crm');
  ...
  $desc.=$feed['object'];
  if($feed['status'] == 5){
   $desc=__("Contact removed from all lists",'contact-form-ccontact-crm');   
  }
  }else if($feed['status'] == 4){
      $desc=sprintf(__("%s Filtered",'contact-form-ccontact-crm'),$feed['object']); 
  }else{
  $desc= !empty($feed['error']) ? $feed['error'] : __("Unknown Error",'contact-form-ccontact-crm');
  }
  $title=__("Failed",'contact-form-ccontact-crm');   
  if($feed['status'] == 1){
  $title=__("Created",'contact-form-ccontact-crm');   
  }else if($feed['status'] == 2){
  $title=__("Updated",'contact-form-ccontact-crm');   
  }else if($feed['status'] == 4){
  $title=__("Filtered",'contact-form-ccontact-crm');   
  }else if($feed['status'] == 5){
  $title=__("Deleted",'contact-form-ccontact-crm');   
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.$this->id.'_update" id="'.$this->id.'_update" value="1" /><label for="'.$this->id.'_update" title="%s">%s</label><br /><br />', __('Create or update this entry in Constant Contact. The fields will be mapped according to the form feed settings.', 'contact-form-ccontact-crm'), __('Send to Constant Contact', 'contact-form-ccontact-crm'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Constant Contact Logs','contact-form-ccontact-crm').'" href="'.$log_url.'">'.__('Constant Contact Logs','contact-form-ccontact-crm').'</a>';
...
public static function clean($var){
  ...
  */
  ...
  $status=0;
  if(class_exists('vxcf_ccontact_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/contact-form-ccontact-crm/contact-form-ccontact-crm.php')) {
  $status=2;
  } 
  return $status;
  }
...
public function do_actions(){
  ...
  */
  ...
  $this->data=new vxcf_ccontact_data();
  return $this->data;
  }
...
$account=$this->post('account',$feed);
  ...
  if($this->post('id',$res)!=""){ 
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Constant Contact (%s) with ID # %s .', 'contact-form-ccontact-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Constant Contact (%s) with ID # %s . View entry at %s', 'contact-form-ccontact-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Constant Contact', 'contact-form-ccontact-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Constant Contact (%s) with ID # %s . View entry at %s', 'contact-form-ccontact-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Constant Contact (%s) with ID # %s .', 'contact-form-ccontact-crm'),$feed['object'],$res['id']);   
            }
          if($event == 'delete'){  
     $screen_msg=__( 'Entry deleted from Constant Contact', 'contact-form-ccontact-crm');
          }else{
     $screen_msg=__( 'Entry updated in Constant Contact', 'contact-form-ccontact-crm');
          }
          }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'contact-form-ccontact-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Constant Contact. Entry not sent! Check the Entry Notes below for more details.' , 'contact-form-ccontact-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Constant Contact. Entry not sent' , 'contact-form-ccontact-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'contact-form-ccontact-crm'),$feed['object']);
  if($this->post('error',$res)!=""){
      $error= is_array($res['error']) ? json_encode($res['error']) : $res['error'];
  $msg.=" ($error)";
  
  $_REQUEST['VXGConstant ContactError']=$msg; //front end form error for admin only
  }   
  if(!$is_admin){
      $info['msg']=$msg;
...
//  } 
  ...
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','contact-form-ccontact-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//$this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Constant Contact','contact-form-ccontact-crm')." Error","info_title"=>"More Detail","info"=>$detail);
...
$vxcf_ccontact=new vxcf_ccontact();
$vxcf_ccontact->instance();
$vx_cf['vxcf_ccontact']='vxcf_ccontact';
endif;
