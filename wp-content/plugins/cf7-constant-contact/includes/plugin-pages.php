...
if( !class_exists( 'vxcf_ccontact_pages' ) ) {
...
* @since       1.0.0
*/
class vxcf_ccontact_pages   extends vxcf_ccontact{
    public $ajax=false;
...
*/
 ...
 /*     if(isset($_GET['vxx'])){
       $info=$this->get_info(1); 
    var_dump($info); die();   
  $api=$this->get_api($info);
  $fields=$api->get_lists(''); die();
  
    $id='9658';
 // $this->delete_note('61',$id); die();
  $info=$this->get_cf_entry($id);
  $lead=$this->get_cf_entry_detail($id);
$lead['__vx_id']=$id;
//$this->trash_leads(array($id)); 
$push=$this->push($lead,array('id'=>'cf_6'));
    var_dump($push); die();
}*/
...
//
 ...
 add_filter("admin_init", array($this, 'setup_plugin'));
  $this->tooltips();
 }
...
public function add_bulk_send_crm_callcenter($list){ 
 $list['send_ccontact']=__('Send to Constant Contact','contact-form-ccontact-crm');
  return $list;  
}
public function bulk_send_crm_callcenter($msg,$action,$ids,$type){ 
    if($action == 'send_ccontact'){
...
$entry['__vx_id']=$entry['id'];
        ...
        if(empty($notice)){
            $notice='Nothing Sent to Constant Contact';
        }
...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo $this->id ?>_send" value="yes"> <?php _e('Send to Constant Contact','contact-form-ccontact-crm') ?></label>
  </div>
  <?php  
}
    /**
  * Display custom notices
  * show ccontact response
  * 
  */
...
$push=$this->push($lead,$form);
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'contact-form-ccontact-crm') . '</a>' );
        }
        return $links;
   }
  /**
  ...
  */
  ...
  // Adding submenu if user has access
        $page_title =__('Constant Contact','contact-form-ccontact-crm');
        $menu_title =__('Constant Contact','contact-form-ccontact-crm');
...
die();
 ...
 if(isset($_REQUEST[$this->id.'_tab_action']) && $_REQUEST[$this->id.'_tab_action']=="get_token"){
  ...
  $msg=__('You do not have permissions to add token','contact-form-ccontact-crm');
...
$class='';
 if($res){
       $msg=__('Account Deleted Successfully','contact-form-ccontact-crm');
  $class='updated';   
 }else{
       $msg=__('Error While Removing Account','contact-form-ccontact-crm');
  $class='error';      
 }
...
die();
  ...
  $msg=__('You do not have permissions to export logs','contact-form-ccontact-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  ...
  $fields=array(); $field_titles=array("#",__('Status','contact-form-ccontact-crm'),__('Constant Contact ID','contact-form-ccontact-crm') ,__('Entry ID','contact-form-ccontact-crm'),__('Description','contact-form-ccontact-crm'),__('Time','contact-form-ccontact-crm'));
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to clear logs','contact-form-ccontact-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  $clear=$data->clear_logs();
       ...
       $msg=__('Error While Clearing Constant Contact Logs','contact-form-ccontact-crm');
      $level="error";
      if(!empty($clear)){
      $msg=__('Constant Contact Logs Cleared Successfully','contact-form-ccontact-crm');   
      $level="updated";
      }
...
public function tooltips(){
     self::$tooltips = array(
 'vx_feed_name' =>  __('Enter feed name of your choice.', 'contact-form-ccontact-crm'),
  'vx_sel_object' => __('Select the Object to Create when a Form is Submitted.', 'contact-form-ccontact-crm'),
   'vx_sel_account' =>__('Select the Constant Contact account you would like to export entries to.', 'contact-form-ccontact-crm'),
  'vx_sel_form' => __('Select the Contact Form you would like to integrate with Constant Contact. Contacts generated by this form will be automatically added to your Constant Contact account.', 'contact-form-ccontact-crm'),
  
  'vx_map_fields' => __('Associate your Constant Contact fields to the appropriate Contact Form fields.', 'contact-form-ccontact-crm'),
  
  'vx_optin_condition' =>__('When the opt-in condition is enabled, form submissions will only be exported to Constant Contact when the condition is met. When disabled all form submissions will be exported.', 'contact-form-ccontact-crm'),
  
  'vx_manual_export' => __('If you do not want all entries sent to Constant Contact, but only specific, approved entries, check this box. To manually send an entry to Constant Contact, go to Entries, choose the entry you would like to send to Constant Contact, and then click the "Send to Constant Contact" button.', 'contact-form-ccontact-crm'),
  
    'vx_entry_notes' => __('Enable this option if you want to synchronize Contact Form entry notes to Constant Contact Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Constant Contact Object selected in the feed.', 'contact-form-ccontact-crm'),
    
      'vx_primary_key' => __('Which field should be used to update existing objects?', 'contact-form-ccontact-crm'),
      
  
  'vx_disable_logs'=>__('When an order is sent to Constant Contact we store that order information in the database and show it in the Constant Contact Log. Check this box if you do not want to save the exported order information in the logs.','contact-form-ccontact-crm'),
  
'vx_sources'=>__('Get Lead Sources list from ccontact.','contact-form-ccontact-crm'),
'vx_statuses'=>__('Get Lead Status list from ccontact.','contact-form-ccontact-crm'),
'vx_cats'=>__('Get Categories list from ccontact.','contact-form-ccontact-crm'),

'vx_source'=>__('Which Lead Source should be assigned to this object.','contact-form-ccontact-crm'),
'vx_status'=>__('Which Lead Status should be assigned to this object.','contact-form-ccontact-crm'),
'vx_cat'=>__('Which Category should be assigned to this object.','contact-form-ccontact-crm'),

'vx_source_check'=>__('If enabled, a Lead Source will be assigned to the Object','contact-form-ccontact-crm'),
'vx_status_check'=>__('If enabled, a Lead Status will be assigned to the Object','contact-form-ccontact-crm'),
'vx_cat_check'=>__('If enabled, a Category will be assigned to the Object','contact-form-ccontact-crm'),

'vx_assign_opportunity'=>__('Enable this option if you want to assign an opportunity this object.','contact-form-ccontact-crm'),
'vx_sel_opportunity'=>__('Select Opportunity feed. Opportunity created by this feed will be assigned to this object.','contact-form-ccontact-crm'),

'vx_assign_contact'=>__('Enable this option , if you want to assign a Contact to this object','contact-form-ccontact-crm'),
'vx_sel_contact'=>__('Select Contact feed. Contact created by this feed will be assigned to this object','contact-form-ccontact-crm'),

'vx_assign_organization'=>__('Enable this option , if you want to assign a Organization to this object','contact-form-ccontact-crm'),
'vx_sel_organization'=>__('Select Organization feed. Organization created by this feed will be assigned to this object','contact-form-ccontact-crm'),

   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','contact-form-ccontact-crm'),
   'vx_owners'=>__('Get Users list from Constant Contact','contact-form-ccontact-crm'),
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to Constant Contact Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the Constant Contact Object selected in the feed.','contact-form-ccontact-crm'),
   'vx_sel_owner'=>__('Select a user as a owner of this object','contact-form-ccontact-crm'),
   
       'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note.', 'contact-form-ccontact-crm'),
   'vx_note_fields'=>__('Select fields which you want to send as a note', 'contact-form-ccontact-crm'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry', 'contact-form-ccontact-crm')
...
<div class="table_head_i"><?php _e('Capsule', 'contact-form-ccontact-crm'); ?></div>  
<div class="vx_group">
...
echo $this->format_log_msg($log);
 ...
 $msg=__('You do not have permissions for this action','contact-form-ccontact-crm');
...
if( $this->has_feed($form_id)) { 
 $boxes['ccontact_crm']=array('title'=>'<i class="fa fa-plug"></i> '.__('Constant Contact', 'contact-form-ccontact-crm'),'callback'=>array($this,'meta_box_html'));  
} return $boxes; 
}
...
public function has_feed($form_id) {
 ...
 if(!empty($info['meta']) ){
  ...
  ob_start();
  $this->groups(array('object'=>$object),$meta);  
  $data['vx_lists_data']=ob_get_clean();
  $res['data']=$data;   
      break;    
  case"refresh_users":
    ...
    $res['status']="ok";
     ...
     $res['error']=$users;   
        ...
        $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','contact-form-ccontact-crm'));
  $meta['users']=$users;  
  $res['data']=$data;   
      break;
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','contact-form-ccontact-crm');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  public  function mapping_page(){
      ...
      wp_enqueue_style('vxc-tooltip');
         $tabs=array('feeds'=>__('Constant Contact Feeds','contact-form-ccontact-crm') , 'logs'=>__('Constant Contact Log','contact-form-ccontact-crm') , 'accounts'=>__('Constant Contact Accounts','contact-form-ccontact-crm'), 'settings'=>__('Settings','contact-form-ccontact-crm'));
  ...
  $(this).text('<?php _e('Show Key','contact-form-ccontact-crm') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php _e('Hide Key','contact-form-ccontact-crm') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  _e('You do not have permissions to access this page','contact-form-ccontact-crm');    
  return;
  }
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'contact-form-ccontact-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'contact-form-ccontact-crm') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','contact-form-ccontact-crm'),$count));  
    }
  ...
  if(!empty($entry) && is_array($entry)){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','contact-form-ccontact-crm'));  
  }
...
$bulk_actions=array(""=>__('Bulk Action','contact-form-ccontact-crm'),"delete"=>__('Delete','contact-form-ccontact-crm'),
  'send_to_crm_bulk'=>__('Send to Constant Contact','contact-form-ccontact-crm'),'send_to_crm_bulk_force'=>__('Force Send to Constant Contact - Ignore filters','contact-form-ccontact-crm'));
$base_url=$this->get_base_url();
$objects=$this->get_objects(); 
$statuses=array("1"=>__("Created",'contact-form-ccontact-crm'),"2"=>__("Updated",'contact-form-ccontact-crm'),"error"=>__("Failed",'contact-form-ccontact-crm'),"4"=>__("Filtered",'contact-form-ccontact-crm'),"5"=>__("Deleted",'contact-form-ccontact-crm')); 
...
//
  ...
  $allowed=array('first_name','last_name','email_address','email');
         if(!self::$is_pr){
     $temp_fields=array(); $n=0;
...
foreach($map as $field_k=>$field_v){
  ...
  $sel_fields=array(""=>__("Standard Field",'contact-form-ccontact-crm'),"value"=>__("Custom Value",'contact-form-ccontact-crm'));
...
public function groups($meta,$info){
 ...
 if(!empty($info['lists'])){ 
  ...
  <div class="vx_row">
   <div class="vx_col1">
  <label for="crm_sel_camp"><?php echo __('Select List(s)','contact-form-ccontact-crm') ?></label>
</div> <div class="vx_col2">
<?php
foreach($info['lists'] as $k=>$l){
$check=''; 
if($meta['object'] == $k){ $check.=' disabled="disabled"'; } 

if(!empty($meta['lists'][$k]) || !empty($check)){
$check.='checked="checked"';    
}  

?><div class="vx_check_item">
<input type="checkbox" <?php echo $check ?> id="meta_item_<?php echo $k ?>" name="meta[lists][<?php echo $k ?>]" value="1">
<label for="meta_item_<?php echo $k ?>"><?php echo $l; ?></label>
</div><?php
}        
?>
</div>
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = __('Constant Contact','contact-form-ccontact-crm');
        return $plugins;
    }
  /**
...
$this->ajax=true;
  ...
  $msg=__("Please Choose Object",'contact-form-ccontact-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-ccontact-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'contact-form-ccontact-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-ccontact-crm');
  }
  if($msg !=""){
...
$field_options="<option>".__("Select Object",'contact-form-ccontact-crm')."</option>"; 
  if(is_array($objects)){
...
die();
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','contact-form-ccontact-crm');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Contact Form Constant Contact Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'contact-form-ccontact-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  $meta=isset($_POST['meta']) ? $this->post('meta') : array();
   ...
   $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-ccontact-crm'));
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  _e('You do not have permissions to save settings','contact-form-ccontact-crm');
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-ccontact-crm'));
  $valid_email=true;
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','contact-form-ccontact-crm'));
  }
   $info=$this->get_info($id); 
   $crm=$this->post('crm');
...
//  $force_check=true;
  ...
  if($con_class != "updated" ){
      $msg=__('Connection to Constant Contact is NOT Working','contact-form-ccontact-crm');  
  }else{
     $msg=__('Connection to Constant Contact is Working','contact-form-ccontact-crm');   
  }
  $title=__('Test Connection: ','contact-form-ccontact-crm');
...
if(isset($_POST[$this->id."_submit"])){ 
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  _e('You do not have permissions to edit/save feed','contact-form-ccontact-crm'); 
  return;
  }
  //
...
if(is_array($feed_update) && is_array($feed)){
  ...
  if($is_valid){
      ...
      $msgs['save']=array('class'=>'updated','msg'=>sprintf(__("Feed Updated. %sback to list%s", 'contact-form-ccontact-crm'), '<a href="'.$feed_link.'">', "</a>"));
  }
  else{
  $msgs['save']=array('class'=>'error','msg'=>__("Feed could not be updated. Please enter all required information below.", 'contact-form-ccontact-crm'));
}
} 
    //getting  API
...
if(is_array($tags)){
  ...
  $fields['__vx_id']=array('id'=>'__vx_id','label'=>__('Entry ID','contact-form-ccontact-crm')); 
$fields['_vx_form_id']=array('id'=>'_vx_form_id','label'=>__('Form ID','contact-form-ccontact-crm')); 
$fields['_vx_form_name']=array('id'=>'_vx_form_title','label'=>__('Form Title','contact-form-ccontact-crm')); 

  $this->fields=$fields=array('cf'=>array("title"=>__('Contact Form Fields','contact-form-ccontact-crm'),"fields"=>$fields));
...
if(is_array($fields_arr['fields'])){
  ...
  */
  ...
  $modules=array(""=>__("Select Object",'contact-form-ccontact-crm'));
  ...
  */
  ...
  $msg=__( 'Successfully Connected to Constant Contact','contact-form-ccontact-crm' );
     if(isset($info['_time'])){
